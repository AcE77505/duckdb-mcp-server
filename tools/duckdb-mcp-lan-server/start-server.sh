...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --enable-dns-rebinding-protection) ENABLE_DNS_REBINDING_PROTECTION="$2"; shift 2 ;;
    -h|--help)
      cat <<'USAGE'
...
installed_hash=""
[[ -f "$REQUIREMENTS_HASH_PATH" ]] && installed_hash="$(tr -d '[:space:]' < "$REQUIREMENTS_HASH_PATH")"
...
PY
  ...
  if [[ $status -ne 0 ]]; then
    ...
    [[ -n "$missing_modules" ]] && echo "Dependency import check failed. Missing modules: $missing_modules" >&2 || echo "Dependency import check failed." >&2
    return 1
  fi
  return 0
}
...
if [[ "$requirements_hash" != "$installed_hash" ]]; then
  needs_install=true
elif ! check_required_modules; then
